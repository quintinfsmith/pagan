...
class ActiveWaveGenerator(var sample_handle_manager: SampleHandleManager) {
    ...
    fun place_events(events: List<MIDIEvent>, frame: Int) {
        ...
        runBlocking {
            this@ActiveWaveGenerator._event_mutex.withLock {
                if (!this@ActiveWaveGenerator._midi_events_by_frame.containsKey(frame)) {
                    this@ActiveWaveGenerator._midi_events_by_frame[frame] = mutableListOf()
                }
                for (event in events) {
                    this@ActiveWaveGenerator._midi_events_by_frame[frame]!!.add(event)
                }
            }
        }
    }
    ...
    fun place_event(event: MIDIEvent, frame: Int, relative: Boolean = false) {
        ...
        runBlocking {
            this@ActiveWaveGenerator._event_mutex.withLock {
                this@ActiveWaveGenerator.last_frame = max(frame, this@ActiveWaveGenerator.last_frame)
                if (!this@ActiveWaveGenerator._midi_events_by_frame.containsKey(frame)) {
                    this@ActiveWaveGenerator._midi_events_by_frame[frame] = mutableListOf()
                }
                this@ActiveWaveGenerator._midi_events_by_frame[frame]!!.add(event)
    ...
    fun update_active_frames(initial_frame: Int, buffer_size: Int) {
         ...
         */
        runBlocking {
            this@ActiveWaveGenerator.event_queue_mutex.withLock {
                if (this@ActiveWaveGenerator.active_event_queue.isNotEmpty()) {
                    val size = this@ActiveWaveGenerator.active_event_queue.size
                    for (i in 0 until size) {
                        val pair = this@ActiveWaveGenerator.active_event_queue.removeFirst() ?: break
                        var (f, event) = pair
                        this@ActiveWaveGenerator.place_event(event, f + initial_frame + buffer_size)
                    }
                }
            }
        }
    ...
    fun clear() {
        ...
        runBlocking {
            this@ActiveWaveGenerator.event_queue_mutex.withLock {
                this@ActiveWaveGenerator.active_event_queue.clear()
            }
            this@ActiveWaveGenerator._event_mutex.withLock {
                this@ActiveWaveGenerator._midi_events_by_frame.clear()
            }
        }
        this.frame = 0
        this._empty_chunks_count = 0

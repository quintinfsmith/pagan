    ...
    class WaveGenerator(private var player: SoundFontWavPlayer) {
       ...
       // }
        ...
        fun process_event(event: MIDIEvent) {
            runBlocking {
                this@WaveGenerator.event_mutex.withLock {
                    val f = this@WaveGenerator.frame + AudioTrackHandle.buffer_size
        ...
        fun generate(): ShortArray {
            ...
            if (!this.listening && (this.max_frame > 0 && this.frame >= this.max_frame && this.active_sample_handles.isEmpty())) {
                this.is_alive = false
            }

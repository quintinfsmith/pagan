...
class ContextMenuLine(primary_container: ViewGroup, secondary_container: ViewGroup): ContextMenuView(R.layout.contextmenu_row, R.layout.contextmenu_row_secondary, primary_container, secondary_container) {
    ...
    private lateinit var _button_choose_percussion: TextView
    private lateinit var _button_toggle_volume_control: ImageView
    private lateinit var _widget_volume: ControlWidgetVolume
    private lateinit var _spacer: Space
    private val _visible_line_controls_domain = listOf(ControlEventType.Volume, ControlEventType.Pan)
    ...
    override fun init_properties() {
        ...
        this._button_toggle_volume_control = primary.findViewById(R.id.btnToggleChannelCtl)
        this._button_insert = primary.findViewById(R.id.btnInsertLine)
        this._button_remove = primary.findViewById(R.id.btnRemoveLine)
        this._button_choose_percussion = primary.findViewById(R.id.btnChoosePercussion)

        this._widget_volume = ControlWidgetVolume(OpusVolumeEvent(0F), true, this.context) { event: OpusControlEvent ->
        ...
        this.secondary!!.addView(this._widget_volume)
        (this._widget_volume as View).layoutParams.width = ViewGroup.LayoutParams.MATCH_PARENT
        (this._widget_volume as View).layoutParams.height = ViewGroup.LayoutParams.MATCH_PARENT

        this._spacer = primary.findViewById(R.id.spacer)
    }
    ...
    override fun refresh() {
        ...
        if (!opus_manager.is_percussion(channel)) {
            this._spacer.visibility = View.VISIBLE
            this._button_choose_percussion.visibility = View.GONE
        } else {
            this._spacer.visibility = View.GONE
            this._button_choose_percussion.visibility = View.VISIBLE
            val instrument = opus_manager.get_percussion_instrument(line_offset)
            main.populate_active_percussion_names(false)
            this._button_choose_percussion.text = if (this.context.resources.configuration.orientation == Configuration.ORIENTATION_LANDSCAPE) {
        ...
        this._button_remove.isEnabled = working_channel.size > 1
        ...
        if (!show_control_toggle) {
            this._button_toggle_volume_control.visibility = View.GONE
        } else {
            this._button_toggle_volume_control.visibility = View.VISIBLE
        }
        ...
        this._widget_volume.set_event(controller.initial_event, true)
        this._widget_volume.visibility = View.VISIBLE

    }

    private fun dialog_popup_hidden_lines() {
        val opus_manager = this.get_opus_manager()
    ...
    override fun setup_interactions() {
        this._button_choose_percussion.setOnClickListener {
            if (!it.isEnabled) {
        ...
        this._button_toggle_volume_control.setOnClickListener {
            if (!it.isEnabled) {
                return@setOnClickListener
            }
    ...
    private fun click_button_insert_line() {
        val main = this.get_main()
        val opus_manager = main.get_opus_manager()
        opus_manager.insert_line_at_cursor(1)
    }

    private fun long_click_button_insert_line(): Boolean {
        val main = this.get_main()
        val opus_manager = main.get_opus_manager()
    ...
    private fun click_button_remove_line() {
        val main = this.get_main()
        val opus_manager = main.get_opus_manager()
        opus_manager.remove_line_at_cursor(1)
    }

    private fun long_click_button_remove_line(): Boolean {
        val main = this.get_main()
        val opus_manager = main.get_opus_manager()

...
class OpusLayerInterface : OpusLayerCursor() {
   ...
   override fun clear_link_pool(beat_key: BeatKey) {
    ...
    fun calculate_overlapping_leafs(beat_key: BeatKey, position: List<Int>): List<Pair<BeatKey, List<Int>>> {
        ...
        val output = mutableListOf<Pair<BeatKey, List<Int>>>()
        while (true) {
            ...
            val (next_offset, _) = this.get_leaf_offset_and_width(next_beat_key, next_position)
            val adj_offset = (next_beat_key.beat - beat_key.beat) - (1f - target_offset)
            if (next_offset + adj_offset <= (target_offset + duration_width)) {
                output.add(Pair(next_beat_key, next_position))
            } else {
                break
            }
        }

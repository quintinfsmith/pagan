...
class UIChangeBill {
    ...
    var full_refresh_flagged = false
    private val _int_queue = mutableListOf<Int>()

    fun get_next_entry(): BillableItem? {
        return if (this.full_refresh_flagged) {
            this.full_refresh_flagged = false
            BillableItem.FullRefresh
        } else if (this.bill.isEmpty()) {
            null
        } else {
    ...
    fun queue_full_refresh() {
        this.full_refresh_flagged = true
        this.bill.clear()
        this._int_queue.clear()
    }

    fun queue_project_name_change() {
        if (this.full_refresh_flagged) {
            return
        }

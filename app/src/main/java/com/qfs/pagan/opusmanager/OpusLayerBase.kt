...
open class OpusLayerBase {
    ...
    fun get_channel_ctl_proceding_leaf_position(ctl_type: ControlEventType, channel: Int, beat: Int, position: List<Int>): Pair<Int, List<Int>>? {
        return this.channels[channel].controllers.get_controller(ctl_type).get_proceding_leaf_position(beat, position)
    }

    fun get_line_ctl_proceding_leaf_position(ctl_type: ControlEventType, beat_key: BeatKey, position: List<Int>): Pair<Int, List<Int>>? {
        return this.channels[beat_key.channel].lines[beat_key.line_offset].controllers.get_controller(ctl_type).get_proceding_leaf_position(beat_key.beat, position)
    }
    ...
    private fun copy_func(tree: OpusTree<InstrumentEvent>): InstrumentEvent? {
    ...
    open fun get_midi(start_beat: Int = 0, end_beat_rel: Int? = null): Midi {
        ...
        this.channels.forEachIndexed outer@{ c: Int, channel: OpusChannel ->
            ...
            channel.lines.forEachIndexed inner@{ l: Int, line: OpusLine ->
                ...
                var current_tick = 0
                var prev_note = 0
                line.beats.forEachIndexed { b: Int, beat: OpusTree<InstrumentEvent> ->
                    val stack: MutableList<StackItem<InstrumentEvent>> = mutableListOf(StackItem(beat, 1, current_tick, midi.ppqn))
                    while (stack.isNotEmpty()) {
                        val current = stack.removeFirst()
                        if (current.tree.is_event()) {
                            val event = current.tree.get_event()!!
    ...
    open fun save(path: String? = null) {
        ...
        val generalized_object = ParsedHashMap(
            hashMapOf(
                "d" to OpusManagerGeneralizer.generalize(this),
                "v" to ParsedInt(3)
            )
        )
    ...
    private fun tree_from_midi(midi: Midi): Triple<OpusTree<Set<InstrumentEvent>>, List<OpusTree<OpusControlEvent>>, List<Triple<Int, Int?, Int?>>> {
        var beat_size = midi.get_ppqn()
        var total_beat_offset = 0
        var last_ts_change = 0
        val beat_values: MutableList<OpusTree<Set<InstrumentEvent>>> = mutableListOf()
        ...
        for (pair in midi.get_all_events()) {
            ...
            if (event is NoteOn && event.get_velocity() > 0) {
                ...
                while (beat_values.size <= beat_index) {
                    val new_tree = OpusTree<Set<InstrumentEvent>>()
        ...
        val opus = OpusTree<Set<InstrumentEvent>>()
    ...
    private fun _copy_channel_ctl_range(type: ControlEventType, target_channel: Int, target_beat: Int, original_channel: Int, start: Int, end: Int, unset_original: Boolean) {
        ...
        for (i in start .. end) {
            overwrite_map[target_beat + (i - start)] = original_controller.get_tree(i).copy()
            if (unset_original) {
    ...
    private fun _copy_line_ctl_range(type: ControlEventType, beat_key: BeatKey, first_corner: BeatKey, second_corner: BeatKey, unset_original: Boolean) {
        ...
        for (o_key in original_keys) {
            trees.add(this.get_line_ctl_tree(type, o_key).copy())
        }

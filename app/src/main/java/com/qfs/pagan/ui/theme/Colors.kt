        ...
        val background = if (is_muted) {
            if (dark_mode) {
                when (active) {
                    ...
                    LeafState.Spill -> {
                        when (selected) {
                            LeafSelection.Primary -> MUTED_LEAF_SELECTED_NIGHT
                            LeafSelection.Secondary -> MUTED_LEAF_SECONDARY_NIGHT
                            LeafSelection.Unselected -> MUTED_LEAF_COLOR_NIGHT
                        }
                    }
                    LeafState.Empty -> {
                        when (selected) {
                            LeafSelection.Primary -> MUTED_LINE_SELECTED_NIGHT
                            LeafSelection.Secondary -> MUTED_LINE_SELECTED_NIGHT
                ...
                when (active) {
                    ...
                    LeafState.Spill -> {
                        when (selected) {
                            LeafSelection.Primary -> MUTED_LEAF_SELECTED
                            LeafSelection.Secondary -> MUTED_LEAF_SECONDARY
                            LeafSelection.Unselected -> MUTED_LEAF_COLOR
                        }
                    }
                    LeafState.Empty -> {
                        when (selected) {
                            LeafSelection.Primary -> MUTED_LINE_SELECTED
                            LeafSelection.Secondary -> MUTED_LINE_SELECTED // TODO?
            ...
            when (active) {
                LeafState.Active -> {
                    when (selected) {
                        LeafSelection.Primary -> EFFECT_COLOR_SELECTED
                        LeafSelection.Secondary -> EFFECT_COLOR_SECONDARY
                        LeafSelection.Unselected -> EFFECT_COLOR
                    }
                }
                LeafState.Spill -> {
                    when (selected) {
                        LeafSelection.Primary -> EFFECT_COLOR_SELECTED
                        LeafSelection.Secondary -> EFFECT_COLOR_SECONDARY
                        LeafSelection.Unselected -> EFFECT_COLOR_SPILL
                    }
                }
                LeafState.Empty -> {
                    if (dark_mode) {
            ...
            when (active) {
                LeafState.Active -> {
                    when (selected) {
                        LeafSelection.Primary -> LEAF_COLOR_SELECTED
                        LeafSelection.Secondary -> LEAF_COLOR_SECONDARY
                ...
                LeafState.Spill -> {
                    when (selected) {
                        LeafSelection.Primary -> LEAF_COLOR_SELECTED
                        LeafSelection.Secondary -> LEAF_COLOR_SECONDARY
                        LeafSelection.Unselected -> LEAF_COLOR_SPILL
                    }
                }
                LeafState.Empty -> {
                    if (dark_mode) {

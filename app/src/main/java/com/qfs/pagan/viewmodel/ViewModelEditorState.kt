...
class ViewModelEditorState: ViewModel() {
    ...
    fun get_active_zoom(): Float {
        return this.active_zoom.floatValue
    }
    ...
    fun update_global_zoom_notches(x: Int? = null) {
        val current_index = this.zoom_index.intValue
        val current_value = this.active_zoom.floatValue


        val pegs = if (x == null) {
            mutableSetOf(1F).also {
                for (column in this.column_data)  {
                    it.addAll(column.zoom_notches)
                }
            }
        } else {
            this.zoom_notches.toMutableSet().also {
                it.addAll(this.column_data[x].zoom_notches)
            }
        }
        ...
        this.max_zoom_index.intValue = max(0, this.zoom_notches.size - 1)
        if (current_index == 0) {
            this.active_zoom.floatValue = this.zoom_notches.first()
        } else if (this.zoom_notches.contains(current_value)) {
            this.zoom_index.intValue = this.zoom_notches.indexOf(current_value)
        } else {
    ...
    fun add_channel(channel: Int, percussion: Boolean, instrument: PresetKey, is_mute: Boolean, size: Int = 0, palette: OpusColorPalette) {
    ...
    fun set_channel_data(channel_index: Int, percussion: Boolean, preset: PresetKey, is_mute: Boolean, size: Int = 0, palette: OpusColorPalette) {
    ...
    fun populate_presets(index: Int, soundfont: SoundFont) {
        ...
        for ((name, bank, program) in soundfont.get_available_presets()) {
            ...
            this.available_instruments[PresetKey(index, bank, program)] = available_keys.sortedBy { it.second }
        }
    }

...
import radixulous.app.layer_flag.FlagLayer

class LinksLayer : FlagLayer {
    constructor {
        super()
        var linked_beat_map: HashMap<BeatKey, BeatKey> = HashMap<BeatKey, BeatKey>();
        var inv_linked_beat: HashMap<BeatKey, List<BeatKey>> = HashMap<BeatKey, List<BeatKey>>();
    }
    ...
    fun unlink_beat(beat_key: BeatKey) {
        ...
        var beats: List<BeatKey> = this.inv_linked_beat_map.get(target_key)
    ...
    fun clear_links_to_beat(beat_key: BeatKey) {
        if (! self.inv_linked_beat_map.containsKey(beat_key)) {
            return
        }
    ...
    open public fun split_tree(beat_key: BeatKey, position: List<Int>, splits: Int) {
        for (linked_key in this.get_all_linked(beat_key)) {
            super.split_tree(linked_key, position)
        }
    }
    ...
    private fun remap_links(remap_hook: String, args: List<Int>) {
    ...
    open public fun change_line_channel(old_channel: Int, Line_offset: Int, new_channel: Int) {
    ...
    private fun rh_change_line_channel(beat_key: BeatKey, args: List<Int>) -> BeatKey? {
        ...
        if (beat_key.channel == old_channel) {
            if (beat_key.line_offset == line_offset) {
                new_beat = BeatKey(new_channel, new_offset, bat.beat)
    ...
    private fun rh_insert_beat(beat: BeatIndex, args: List<Int>): BeatKey? {
        var index = args[0]
        var new_beat: BeatIndex
        if (beat.beat >= index) {
    ...
    open public fun remove_beat(index: Int) {
        super.remove_beat(index)
        this.remap_links(this.th_remove_beat, listOf(index))
    }
    private fun rh_remove_beat(beat: BeatIndex, args: List<Int>): BeatKey? {
        var index = args[0]
        var new_beat: BeatIndex
    ...
    open public fun remove_channel(channel: Int) {
        ...
        this.remap_links(this.rh_remap_links, listOf(channel))
    }
    ...
    open public fun remove_line(channel: Int, line_offset: Int? = null) {
    ...
    open public fun swap_channels(channel_a: Int, channel_b) {

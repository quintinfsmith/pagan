...
from typing import Optional, List, Tuple, Dict, TypeVar, Generic, Set
...
class MissingCommaError(Exception):
    """Thrown when OpusTree[MIDITreeEvent].from_string() fails"""
    def __init__(self, repstring, fail_index, beat):
        ...
        msg += f"You likely missed a \"{CH_NEXT}\" Here:\n"
...
@staticmethod
def opustree_from_string(repstring: str, **kwargs) -> OpusTree[MIDITreeEvent]:
    ...
    output: OpusTree[MIDITreeEvent] = OpusTree()

    tree_stack: List[OpusTree[MIDITreeEvent]] = [output]
    register: List[Optional[int], Optional[int], Optional[float]] = [None, None, 0]
    opened_indeces: List[int] = []
    relative_flag: Optional[str] = None
    repeat_queue: List[OpusTree[MIDITreeEvent]] = []
    ...
    for i, character in enumerate(repstring):
        ...
        #elif relative_flag == OpusTree[MIDITreeEvent].CH_REPEAT:
        #    if character == OpusTree[MIDITreeEvent].CH_REPEAT:
        #        repeat_length = 1
        #    else:
        ...
        #    relative_flag = None
            ...
            elif relative_flag == CH_ADD:
                odd_note += int(character, base)
            elif relative_flag == CH_UP:
                odd_note += int(character, base) * base
            elif relative_flag == CH_DOWN:
                odd_note -= int(character, base) * base
        ...
        elif character in REL_CHARS:
            relative_flag = character

        elif character not in SPECIAL_CHARS:
    ...
    for i, beat in enumerate(output):
        if beat.is_leaf():
            new_tree = OpusTree[MIDITreeEvent]()
            new_tree.set_size(1)
...
def opustree_to_string(tree: OpusTree[MIDITreeEvent], base=12) -> str:
    if tree.is_open():
        output = "__"
    elif tree.is_event():
        output = ""
        ...
        if event.relative:
            ...
            if event.note == 0 or event.note % base != 0:
                if event.note < 0:
                    new_string += tree.CH_SUBTRACT
                else:
                    new_string += tree.CH_ADD
                new_string += get_number_string(int(math.fabs(event.note)), base, digit_count=1)
            else:
                if event.note < 0:
                    new_string += tree.CH_DOWN
                else:
                    new_string += tree.CH_UP
        ...
        for i in range(len(tree)):
            ...
            if len(subtree) > 1:
                str_rep = f"{CH_OPEN}{str_rep}{CH_CLOSE}"
            strreps.append(str_rep)

        output = tree.CH_NEXT.join(strreps)

    return output

@staticmethod
def opustree_from_midi(midi) -> OpusTree[MIDITreeEvent]:
    beat_size = midi.ppqn
    total_beat_offset = 0
    last_ts_change = 0
...
def opustree_to_midi(tree: OpusTree[Set[MIDITreeEvent]], **kwargs) -> MIDI:
    transpose = kwargs.get('transpose', 0)
    tempo = int(kwargs.get('tempo', 120))

    midi = MIDI()
    for i in range(16):
        instrument = kwargs.get(f"i{i}", 0)
        midi.add_event(ProgramChange(channel=i, program=instrument), tick=0, track=0)

    midi.add_event(SetTempo.from_bpm(tempo), tick=0, track=0)

    current_tick = 0
    prev_note = 0
    for m in range(tree.size):
        beat = tree.get(m)
        if beat.is_event():
            for event in beat.get_event():
                ...
                midi.add_event(
                    NoteOn(channel=channel, note=note, velocity=64),
                    track=0,
                    tick=current_tick
                )
                midi.add_event(
                    NoteOff(channel=channel, note=note, velocity=64),
                    track=0,
                    tick=current_tick + midi.ppqn
                )
        elif not beat.is_leaf():
            if not beat.is_flat():
                beat.flatten()
            beat.reduce()
            beat.flatten()
            div_size = midi.ppqn // beat.size
            for i in range(beat.size):
                ...
                for event in leaf.get_event():
                    ...
                    midi.add_event(
                        NoteOn(channel=channel, note=note, velocity=64),
                        track=0,
                        tick=current_tick + (i * div_size),
                    )
                    midi.add_event(
                        NoteOff(channel=channel, note=note, velocity=64),
                        track=0,
                        tick=current_tick + ((i + 1) * div_size),
                    )
